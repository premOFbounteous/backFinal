    ...
    try {
        ...
        for (const item of cart.items) {
            ...
            if ((product.stock ?? 0) < (item.quantity ?? 0)) {
                return res.status(400).json({ detail: `Not enough stock for ${product.title}` });
            }
            ...
            order_items.push({
                ...
                thumbnail: product.thumbnail // <-- ADDED thumbnail to the order item
            });
        ...
        const orders = db.collection<OrderDoc>('orders');
        // Use Omit for better type safety, as _id is generated by MongoDB
        const orderDoc: Omit<OrderDoc, '_id'> = {
            user_id,
            items: order_items,
            total,
            status: 'pending',
            createdAt: new Date() // <-- ADDED the current date and time
        };
